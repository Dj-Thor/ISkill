...
export default function PlaylistPage({
  ...
  const [comments, setComments] = React.useState(
    Data && Data.comments ? Data.comments.reverse() : [],
  );
  ...
  const addComment = async () => {
    let response = await fetch(`${URL}/api/addComment`, {
      ...
      body: JSON.stringify(
        { comment: comment.textArea, courseId: courseId },
        null,
        2,
      ),
    });
  ...
  return (
    <>
      ...
      <section className="min-h-screen">
        <h1 className="myColor1 text-3xl title-font font-medium mb-2 mt-12 w-full px-4 md:w-3/4 mx-auto">
          {name}
        </h1>
        <Player id={current} />
        {isVideo && (
        ...
        <div className="m-4  lg:w-3/4 lg:mx-auto">
          <Accordion open={open1 === 1} icon={<Icon id={1} open={open1} />}>
            <AccordionHeader
              className=" font-lato myColor1 text-2xl"
            ...
            <AccordionBody className=" transition transition-600 ease-in-out text-sm font-poppins">
              <div className="lg:w-4/5 mx-auto flex flex-wrap ">
                ...
                <div className="w-full lg:pl-10 lg:py-6 md:mt-6 pt-2 lg:mt-0 px-5 ">
                  <h2 className="text-sm title-font text-gray-500 tracking-widest text-pink-500">
                    COURSE NAME
                  </h2>
                  <h1 className="myColor text-3xl title-font font-medium mb-1">
                    {name}
                  </h1>
                  <div className="flex mb-2">
                    ...
                    <span className="flex text-xl items-center my-1">Free</span>
                  </div>
                  <div className="flex mb-2">
          ...
          <Accordion open={open2 === 1} icon={<Icon id={1} open={open2} />}>
            ...
            <AccordionBody className=" transition transition-600 ease-in-out text-sm font-poppins">
              <div className="relative md:w-[50rem] w-full p-4  font-lato mx-auto mt-3 rounded">
                <h3 className="text-xl my-1 font-poppins text-pink-600">
                ...
                <div className="flex w-full justify-between py-1.5">
                  <div className="flex gap-2">
                    <Button
                      ...
                      color="pink"
                      size="sm"
                      className="rounded-md font-lato"
                    >
                      Add Comment
                    </Button>
                  </div>
              ...
              <div className="relative md:w-[50rem] w-full p-4 font-lato mt-2 mx-auto">
                ...
                <div className="overflow-x-scroll h-[40rem]  my-2 rounded p-1">
                  {comments.map((item: string, idx: number) => {
                    return (
                      <div className="my-6 border rounded-lg p-2" key={idx}>
                        <Avatar
                          className="my-3 ms-2"
                          src="https://docs.material-tailwind.com/img/face-2.jpg"
                          alt="avatar"
                        />
                        <span className="ms-2">Anonymous User</span>
                        <p className="p-6 text-sm bg-pink-400 rounded-lg text-white">
                          {item}
                        </p>
                      </div>
                    );
  ...
  try {
    ...
    if (courseInfo.isVideo) {
      const response = await fetch(
        `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&id=${courseInfo.playlistId}&key=${apiKey}`,
      );
      const json = await response.json();
      ...
      return {
        props: {
          Data: JSON.parse(JSON.stringify(courseInfo)) ?? [],
          isVideo: !courseInfo.isVideo,
          currentVideo: videoInfo.id,
          playList: [],
        },
      };
    } else {
      ...
      const response = await fetch(
        `https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=${videoCount}&playlistId=${courseInfo.playlistId}&key=${apiKey}`,
      );
      ...
      return {
        props: {
          Data: JSON.parse(JSON.stringify(courseInfo)) ?? [],
          isVideo: !courseInfo.isVideo,
          currentVideo: playList[0].resourceId.videoId,
          playList: playList,
        },
      };
    }
