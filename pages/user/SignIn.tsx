...
const [signIn, setSignIn] = React.useState({ email: "", password: "", otp: ""});
...
const resendOTP = async () => {
  const myPromise = new Promise(async (resolve, reject) => {
   ...
   let response = await fetch(`${URL}/api/sendotp`, {
    ...
    body: JSON.stringify({ email: signIn.email, isNew:"no", password:signIn.password}, null, 2)
   });
 ...
 const handleSignIn = async (e: React.FormEvent<HTMLFormElement>, id:string) => {
  e.preventDefault();
  setSpin(true);
const myPromise = new Promise(async (resolve, reject) => {

  const response = await authSignIn(id, { redirect: false, email: signIn.email, password: signIn.password, isNew: "no", otp: signIn.otp, callbackUrl: `${URL}/`});
  ...
  <div className="flex justify-center my-8">
   <Card color="transparent" shadow={false}>
    ...
    {Object.values(providers).map((item, idx) => {
     ...
     switch (item.name) {
      case "Credentials":
       value = verify ? (<form key={idx} onSubmit={(e) => handleSignIn(e, item.id)} className="mt-8 mb-2 w-80 max-w-screen-lg sm:w-96">
      ...
      <BiArrowBack className="text-xl" />
        ...
        <div className="mb-4 flex flex-col gap-6">
         <Input type="text" size="lg" label="OTP" name="otp" required value={signIn.otp} onChange={signIn_OnChange} icon={<MdOutlinePassword className="text-xl text-pink-500" />} color="pink" style={{"fontFamily":"lato"}}/>
        ...
        <div className="mb-4 flex flex-col gap-6">
         <Input type="email" size="lg" label="Email" name="email" required value={signIn.email} icon={<MdEmail className="text-xl text-pink-500" />} onChange={signIn_OnChange} color="pink" style={{"fontFamily":"lato"}} />
         <Input type={type} size="lg" label="Password" name="password" value={signIn.password} required onChange={signIn_OnChange} icon={type=="text"?<FaEyeSlash className="text-xl text-pink-500" onClick={()=>{type=="text"?setType("password"):setType("text")}} />:<FaEye className="text-xl text-pink-500" onClick={()=>{type=="text"?setType("password"):setType("text")}} />} style={{"fontFamily":"lato"}} color="pink" />
                  <Typography
        variant="small"
        color="pink"
      ...
      >
        ...
        <Link href={`${URL}/user/Forgot`}>Forgot Password</Link>
      </Typography>
        </div>
