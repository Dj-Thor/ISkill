...
export default function Cards({courseInfo}:Prop) {
 const {name, img, mentors, description, price, rating, likes, enrolledCount, courseId} = courseInfo;
  ...
  return (
   ...
   <span><del className="text-red-500">₹{price}</del><span className="mx-2 text-green-500">Free</span></span>
...
<Wishlist course={courseId}/>
        ...
        <div className="mb-2 font-lato myColor1  text-sm">
         <FaUserTie className="m-1 inline myColor1"/> {mentors.map((mentor,idx)=>{return <span key={idx}> {mentor} </span>})}
        </div>
        <div className="mb-1 font-lato text-sm myColor1">
         <FaRegCalendarAlt className="m-1 inline myColor1"/> Started From 23 Oct, 2023
        </div>
    ...
    <div className="w-full flex flex-wrap text-white p-1 justify-center ">
     ...
     <button className="rounded-full w-28 font-lato bg-pink-500 block mb-4 py-2 font-lato text-md text-white mx-auto">
        Enroll now
   </button>
      </div>
    </Card>
